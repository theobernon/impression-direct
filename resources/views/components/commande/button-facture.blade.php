@if ($value->facturee === 0)
    <form method="GET" id="[{{$value->noCommande}}]" action="{{route('commande.add.facture',['noCommande'=>$value->noCommande,'noDevis'=>$value->noDevis])}}" >
...
@else
    <form method="GET" id="{{$value->noCommande,$value->noDevis}}" action="{{route('commande.facture',['noCommande'=>$value->noCommande,'noDevis'=>$value->noDevis])}}" >
        <button type="submit" style="border: none;background: none">
